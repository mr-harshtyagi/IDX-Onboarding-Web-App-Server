...
app.get("/findkeys/:uniqueId", function (req, res) {
  Organisation.findOne(
    ...
    function (err, foundOrganisation) {
      if (err) res.send(err);
      else {
...
app.post("/verifysignature", function (req, res) {
  ...
  Organisation.findOne({ id: id }, function (err, foundOrganisation) {
    if (err) res.send(err);
      ...
      if (foundOrganisation) data = foundOrganisation.data.toString();
      else flag = 1;
    }
  });
  ...
  Key.findOne({ id: "2x4e" }, function (err, foundKey) {
    if (err) res.send(err);
      ...
      if (flag === 0) res.send(result);
      else res.send("ID not found in the Database");
    }
  });
});
